const translations = {
  en: {
    ...
    // Project 6
    'p6.badge':          'NLP',
    'p6.title':          'Danish News Sentiment Monitor',
    'projects.p6.desc':  'A live dashboard that reads Danish news in near real time, sorts each story by topic, and tracks the mood — positive, neutral or negative — across the major outlets.',
    'p6.overview':       'A live dashboard that keeps an eye on the Danish news cycle. Every couple of hours it gathers fresh headlines and short summaries from seven of the country\'s biggest news outlets, works out what each story is about, and measures its tone — positive, neutral or negative. The result is a single screen that answers a simple question: which topics are dominating Danish news right now, and how does the mood compare across outlets? Everything is done directly on the Danish text, with no translation step in between.',
    'p6.how.title':      'How It Works',
    'p6.how.1':          'Collects headlines and summaries from seven Danish outlets — five through their public news feeds, and two (DR and TV2) by reading the data their own pages already carry',
    'p6.how.2':          'Cleans up each story, removes duplicates, and saves everything to a cloud database so the history builds up over time',
    'p6.how.3':          'Scores the tone of every story with a Danish language model that labels it positive, neutral or negative',
    'p6.how.4':          'Sorts each story into one of 17 standard news categories (politics, crime, sport, economy and so on) by comparing its meaning to each category — no manual rules required',
    'p6.how.5':          'Writes a one-sentence plain-Danish summary of each top story using a language model that runs entirely on the local machine',
    'p6.how.6':          'Refreshes the whole pipeline automatically every couple of hours so the dashboard always reflects the latest news',
    'p6.features.title': 'What the Dashboard Shows',
    'p6.features.1':     '<strong>News overview:</strong> a banner across the top with the day\'s biggest stories, each boiled down to one plain-Danish sentence.',
    'p6.features.2':     '<strong>Topic word clouds:</strong> the words that most define each category, so you can see at a glance what\'s driving the politics, crime or sport coverage.',
    'p6.features.3':     '<strong>Category breakdown:</strong> a chart showing which topics are getting the most coverage right now.',
    'p6.features.4':     '<strong>Sentiment by source:</strong> a side-by-side comparison of how positive or negative each outlet\'s coverage tends to be.',
    'p6.features.5':     '<strong>Heatmap and article table:</strong> a colour grid of average mood for every source and topic, plus a searchable, filterable list of the underlying articles with links back to the originals.',
    'p6.screenshot.1':   'Main view: the "Nyhedsoverblik" banner of top stories, filters, and news grouped by topic',
    'p6.screenshot.2':   'Topic word clouds: the most characteristic terms for each category',
    'p6.screenshot.3':   'Category breakdown and a positive/neutral/negative comparison across sources',
    'p6.screenshot.4':   'Sentiment heatmap by source and topic, plus a filterable article table',
    'p6.sources.title':  'News Sources',
    'p6.sources':        'The monitor follows seven national outlets: Politiken, Information, Jyllands-Posten, Berlingske and Kristeligt Dagblad through their public news feeds, plus DR and TV2, which no longer offer a usable feed and are instead read straight from the data their own pages publish. A start-up health check pings every source, so a broken feed shows up immediately rather than quietly dropping out.',
    'p6.questions.title':'Questions It Answers',
    'p6.questions.1':    'Which topics are dominating Danish news today, and how has that shifted over the past week?',
    'p6.questions.2':    'Which outlets tend to publish the most negative — or the most positive — coverage?',
    'p6.questions.3':    'Is the mood around a given topic consistent across outlets, or does coverage diverge?',
    'p6.questions.4':    'Which topics suddenly spike — and is one outlet driving it, or all of them at once?',
  },
  ...
  da: {
    ...
    // Project 6
    'p6.badge':          'NLP',
    'p6.title':          'Dansk Nyhedssentiment-monitor',
    'projects.p6.desc':  'Et live-dashboard, der læser danske nyheder i næsten realtid, sorterer hver historie efter emne og følger stemningen — positiv, neutral eller negativ — på tværs af de store medier.',
    'p6.overview':       'Et live-dashboard, der holder øje med den danske nyhedsstrøm. Hver anden time henter det friske overskrifter og korte resuméer fra syv af landets største nyhedsmedier, finder ud af, hvad hver historie handler om, og måler dens tone — positiv, neutral eller negativ. Resultatet er ét skærmbillede, der besvarer et enkelt spørgsmål: hvilke emner dominerer danske nyheder lige nu, og hvordan er stemningen sammenlignet på tværs af medierne? Det hele sker direkte på den danske tekst, uden et oversættelsestrin undervejs.',
    'p6.how.title':      'Sådan virker det',
    'p6.how.1':          'Indsamler overskrifter og resuméer fra syv danske medier — fem via deres offentlige nyhedsfeeds og to (DR og TV2) ved at læse de data, deres egne sider allerede indeholder',
    'p6.how.2':          'Rydder op i hver historie, fjerner dubletter og gemmer det hele i en cloud-database, så historikken bygges op over tid',
    'p6.how.3':          'Vurderer tonen i hver historie med en dansk sprogmodel, der mærker den positiv, neutral eller negativ',
    'p6.how.4':          'Sorterer hver historie ind i en af 17 faste nyhedskategorier (politik, kriminalitet, sport, økonomi og så videre) ved at sammenligne dens betydning med hver kategori — uden manuelle regler',
    'p6.how.5':          'Skriver et resumé på én sætning på almindeligt dansk for hver tophistorie med en sprogmodel, der kører helt lokalt på maskinen',
    'p6.how.6':          'Opdaterer hele pipelinen automatisk hver anden time, så dashboardet altid afspejler de nyeste nyheder',
    'p6.features.title': 'Hvad dashboardet viser',
    'p6.features.1':     '<strong>Nyhedsoverblik:</strong> et banner øverst med dagens største historier, hver kogt ned til én sætning på almindeligt dansk.',
    'p6.features.2':     '<strong>Emneskyer:</strong> de ord, der bedst kendetegner hver kategori, så du med ét blik kan se, hvad der driver dækningen af politik, kriminalitet eller sport.',
    'p6.features.3':     '<strong>Kategorifordeling:</strong> et diagram, der viser, hvilke emner der får mest dækning lige nu.',
    'p6.features.4':     '<strong>Sentiment pr. kilde:</strong> en sammenligning side om side af, hvor positiv eller negativ hvert mediums dækning typisk er.',
    'p6.features.5':     '<strong>Varmekort og artikeltabel:</strong> et farvegitter over den gennemsnitlige stemning for hver kilde og hvert emne, plus en søgbar, filtrerbar liste over de bagvedliggende artikler med links til originalerne.',
    'p6.screenshot.1':   'Hovedvisning: "Nyhedsoverblik"-banneret med tophistorier, filtre og nyheder grupperet efter emne',
    'p6.screenshot.2':   'Emneskyer: de mest kendetegnende ord for hver kategori',
    'p6.screenshot.3':   'Kategorifordeling og en positiv/neutral/negativ-sammenligning på tværs af kilder',
    'p6.screenshot.4':   'Sentiment-varmekort pr. kilde og emne, plus en filtrerbar artikeltabel',
    'p6.sources.title':  'Nyhedskilder',
    'p6.sources':        'Monitoren følger syv landsdækkende medier: Politiken, Information, Jyllands-Posten, Berlingske og Kristeligt Dagblad via deres offentlige nyhedsfeeds, plus DR og TV2, som ikke længere tilbyder en brugbar feed og i stedet læses direkte fra de data, deres egne sider udgiver. Et sundhedstjek ved opstart pinger hver kilde, så en død feed dukker op med det samme i stedet for stille at falde ud.',
    'p6.questions.title':'Spørgsmål det besvarer',
    'p6.questions.1':    'Hvilke emner dominerer danske nyheder i dag, og hvordan har det flyttet sig over den seneste uge?',
    'p6.questions.2':    'Hvilke medier har tendens til at udgive den mest negative — eller mest positive — dækning?',
    'p6.questions.3':    'Er stemningen omkring et givent emne ens på tværs af medierne, eller divergerer dækningen?',
    'p6.questions.4':    'Hvilke emner spiker pludseligt — og er det ét medie, der driver det, eller dem alle på én gang?',
  }
};
